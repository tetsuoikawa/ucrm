...
//props.comment.data[1].articleid

onMounted(() => {
  console.log(props.comment.total)
})
</script>
...
<template>
  ...
  <div class="container px-5 py-24 mx-auto flex flex-col">
    <div class="lg:w-5/6 mx-auto">
      ...
      <ul class=" pb-12 flex justify-center flex-row text-lg font-medium text-center text-gray-500 dark:text-gray-400">
        <li class="mr-2">
  ...
  <p v-if="props.comment.total == 0" class="text-center">コメントされた記事はありません</p>
  <div v-for="(post, index) in props.comment.total" :key="props.comment.total">
    <section class="text-gray-600 body-font">
            ...
            <h2 class="text-gray-900 text-lg title-font font-medium mb-2">
              <a class="inline-flex items-center">
                ...
                <span class="flex-grow flex flex-col pl-4">
                  ...
                  <span class="text-gray-500 text-sm">{{ props.comment.data[index].updated_at }}</span>
                </span>
              </a>
            </h2>
              <Link :href="route('mh.show', {mh: props.comment.data[ index ].articleid })">
                <div class="bg-gray-100 p-4 rounded">
                  <p class="leading-relaxed text-base">{{ props.comment.data[index].comment }}</p>
                </div>
              </Link>
              <Link :href="route('mh.show', {mh: props.comment.data[ index ].articleid }, {mh: props.comment.data[ index ].articleid })">
                <p class="leading-relaxed text-2xl mt-4"><b>{{ props.comment.data[index].articletitle }}</b></p>
              </Link>
          </div>
        </div>
      </div>
